...
type Props = {
    ...
    loginContinueWith: string
    locale: string
}

export default function HeroSwiper({ heroTitle, heroDescription, loginTitle, loginDescription, loginContinueWith, locale }: Props) {
    ...
    useEffect(() => {
        if (slide !== 0) return;
        const timer = setTimeout(() => goToSlide(1, 1), AUTO_ADVANCE_MS);
        return () => clearTimeout(timer);
    }, [slide]);

    const handleDragEnd = (_: unknown, info: { offset: { x: number } }) => {
        const threshold = 60;
        if (info.offset.x < -threshold && slide < 1) goToSlide(1, 1);
        if (info.offset.x > threshold && slide > 0) goToSlide(0, -1);
    };

    return (
        <div className="flex flex-col h-screen overflow-hidden w-full font-sf-pro">
                ...
                {slide === 0 ? (
                    ...
                    >
                        <Image
                            ...
                            height={141}
                        />
                        <h6 className="font-medium text-base leading-none tracking-tight text-center w-full">
                            {heroTitle}
                        </h6>
                        <p className='font-normal text-sm text-center max-w-[30vh]'>
                            {heroDescription}
                        </p>
                        <div className='relative flex items-center justify-center'>
                            ...
                            <Image
                                className='z-10 max-h-[35vh] w-auto'
                        ...
                        <div className="flex flex-col space-y-4 w-full max-w-sm">
                            {providers && Object.values(providers).map((provider) => (
                                <button
                                    ...
                                    className="w-full bg-[rgb(104,82,224)] text-white rounded-full hover:bg-purple-700 font-semibold py-3 px-6 flex items-center justify-center transition shadow-sm"
                                >
                                    {providerIcons[provider.id]}
                                    {loginContinueWith} {provider.name}
                                </button>
                            ))}
                        </div>
                    </motion.div>
            ...
            <div className="flex flex-col items-center gap-3 py-6 z-10">
                <div className="flex gap-2">
                    {[0, 1].map((i) => (
                        <button
                            key={i}
                            onClick={() => goToSlide(i, i > slide ? 1 : -1)}
                            className={`w-3 h-3 rounded-full transition-colors ${slide === i ? 'bg-purple-600' : 'bg-purple-200'}`}
                        />
                    ))}
                </div>
                {slide === 0 && (
